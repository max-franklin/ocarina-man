...
export default function Footer() {
    const Invaders = () => {
        ...
        for (let index = 0; index < 100; index++) {
            if (index % 2 == 0) {
                invaders.push(<Image className="py-2 animate-blocky-bounce" src={spaceInvaderIcon} width={20} height={20} alt=""/>)
            } else {
                invaders.push(<Image className="py-2 animate-blocky-bounce-up" src={spaceInvaderIcon} width={20} height={20} alt=""/>)
            }
        }
        return invaders
    }
    ...
    return (
        <footer className="bg-gradient-to-t from-slate-400 to-slate-100 pb-4">
            ...
            <div className='flex flex-col mx-4 space-y-2 sm:flex sm:flex-row sm:justify-center sm:mx-0 sm:space-y-0'>
                ...
                <button id="subscribeButton" className='hover:cursor-rocket-engaged active:bg-green-300 py-1 sm:px-3 sm:py-2 sm:mx-2 rounded-md bg-green-400 hover:bg-green-200 border border-solid border-green-500'>Subscribe</button>
            </div>
